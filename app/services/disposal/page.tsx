...
import { IDisposal, disposalSchema } from "@/app/validations/Order";
...
const page = () => {
  ...
  const onSubmit: SubmitHandler<IDisposal> = async ({
    firstName,
    lastName,
    email,
    phoneNumber,
  ...
  const serviceArr = [
    ...
    },
    {
      idTag: "disposalDate",
      type: "date",
      placeholder: "Entsorgungstermin",
      error: errors.disposalDate,
      register: register("disposalDate"),
      isTextArea: false,
    },
    {
      idTag: "address",
          ...
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-5 pb-12">
            ...
            >
              <Image
                src={img}
                alt={label}
                className="rounded-lg w-full lg:max-w-[550px] max-w-[400px] max-h-[400px] object-cover border-4 border-transparent hover:border-primary soft"
              />
            </motion.div>
            <motion.div
